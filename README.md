...
- **下一本書**：閱讀至接近結尾時，自動提示同目錄的下一本書（本機與 Google Drive 均支援）
...
| 面板 | 說明 |
|------|------|
| **書庫** | 管理本機 TXT 資料夾，顯示每本書及資料夾的閱讀進度 |
| **最近閱讀** | 列出最近開啟的 50 本書 |
| **Google Drive** | 瀏覽並開啟 Google Drive 上的 TXT 檔案，資料夾顯示完結統計 |
...
| 版本 | 說明 |
|------|------|
| 1.0.3 | 霞鶩文楷字型改為本機內嵌（離線可用）、移除 Cutive Mono、完結門檻統一為 95%、資料夾計數改為已完結數 |
| 1.0.2 | 修正 UTF-8 BOM 偵測：BOM 存在時直接以 UTF-8 解碼，不再回退至 CJK 編碼，避免含 BOM 的混合編碼檔案亂碼 |
| 1.0.1 | 進度同步改為時間戳記比較（`updatedAt`）、重置進度功能、重新整理時強制更新進度快取 |
| 1.0.0 | 初始發布 |
